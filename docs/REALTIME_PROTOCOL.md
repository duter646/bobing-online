...
在线状态按“成员在该房间中的有效连接数”计算。同一成员多个标签页中只要还有一个连接，成员仍显示在线；服务进程重启时所有恢复房间的成员先统一标记为离线，并写入 `room:presence-reset` 事件，避免增量恢复后残留错误的在线状态。断线宽限期结束后，如果当前玩家仍离线，服务端会发出 `game:turn-changed` 并自动越过连续离线席位；全员离线时不推进，任一非当前玩家重连后会继续跳到在线席位。
